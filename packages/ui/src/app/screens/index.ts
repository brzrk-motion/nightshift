/**
 * Built-in nav-rail destinations beyond Home. Each one reads only `entities`
 * and `commands` — the two things every `AppRuntime` already has — so this
 * folder never has to depend on the vibe engine, the plugin host or the
 * automation engine directly. Where a screen needs data only the CLI's
