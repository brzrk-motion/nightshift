import type { EventBus, Json, Unsubscribe } from '@nightshift/core';
...
/** Why an entity changed. Subscribers that only care about writes filter on it. */
export type EntityChangeKind = 'registered' | 'updated' | 'removed';

export interface EntityChange<State extends Json = Json> {
  kind: EntityChangeKind;
  entity: Entity<State>;
  /** The state before the change; absent on `registered`. */
  previous?: State;
}

/** Events the store publishes. `change` fires for every kind. */
export interface EntityEvents extends Record<string, unknown[]> {
  change: [change: EntityChange];
  registered: [entity: Entity];
  updated: [change: EntityChange];
...
export interface EntityStore {
  ...
  /** Replaces the whole state rather than merging into it. */
  ...
  /** The underlying bus, for subscribers that need the change kind. */
  readonly events: EventBus<EntityEvents>;
  /** Drops every entity. Used when the runtime restarts. */
  clear(): void;
}
